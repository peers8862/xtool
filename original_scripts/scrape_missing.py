...
INPUT_FLAGGED  = "x_likes_needs_enrichment.json"  # from audit_likes.py
INPUT_FULL     = "x_likes.json"                   # original full file
OUTPUT_FILE    = "x_likes_enriched.json"           # full file with gaps filled
PROGRESS_FILE  = "scrape_missing_progress.json"
...
SCROLL_PAUSE   = 1.5
STALL_SLEEP    = 1.0
DELAY_BETWEEN  = 3.0  # seconds between tweet page visits
    ...
    if isinstance(data, dict):
        ...
        if rest_id and legacy:
            ...
            if rest_id not in url_map:
                url_map[rest_id] = {"urls": [], "cardUrl": "", "cardTitle": "", "cardDesc": ""}
...
def make_response_handler(url_map):
    def handle_response(response):
        ...
        if not any(k in url for k in [
            "TweetDetail", "TweetResultByRestId", "timeline"
        ]):
            return
        try:
            data = response.json()
...
def main():
    if not Path(INPUT_FLAGGED).exists():
        print("ERROR: " + INPUT_FLAGGED + " not found. Run audit_likes.py first.")
        return
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for idx, tweet in enumerate(remaining):
                ...
                print(
                    "[" + str(idx + 1) + "/" + str(len(remaining)) + "] " + tweet_url
                )
                ...
                try:
                    ...
                    net = url_map.get(tid, {})
                    if net.get("urls") or net.get("cardUrl"):
                        i = tweet_map.get(tweet_url)
                        if i is not None:
                            tweets[i] = apply_network_data(tweets[i], url_map)
                            enriched_count += 1
                            print("  enriched: " + str(tweets[i].get("links", [])))
                    else:
