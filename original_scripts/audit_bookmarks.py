...
INPUT_FILE  = "x_bookmarks.json"
OUTPUT_FILE = "x_bookmarks_needs_enrichment.json"
REPORT_FILE = "x_bookmarks_audit_report.txt"
...
def main():
    ...
    for tweet in tweets:
        if tweet.get("links"):             stats["has_links"]   += 1
        if tweet.get("cardUrl"):           stats["has_card_url"] += 1
    ...
    lines = [
        ...
        "Has media     : " + str(stats["has_media"])   + " (" + str(round(stats["has_media"]    / total * 100, 1)) + "%)",
